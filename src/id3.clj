...
(defn tag->normal [id [frame & more :as frames]]
	(case (common/frame-type id)
		:id3.frame.type/text
			(if more
				(error "Multiple %s frames" id)
				(:id3.frame/content frame))
		:id3.frame.type/url
			(if more
				(error "Mutliple %s frames" id)
				(:id3.frame/url frame))
		:id3.frame.type/user-text
			(->> frames
				(m/index-by :id3.frame/description)
				(m/map-vals :id3.frame/content))
		:id3.frame.type/user-url
			(->> frames
				(m/index-by :id3.frame/description frames)
				(m/map-vals :id3.frame/content))
		:id3.frame.type/picture
			(mapv #(select-keys % [:id3.frame/picture-type :id3.frame/mime-type :id3.frame/bytes]) frames)
		:id3.frame.type/blob
			(mapv :id3.frame/bytes frames)))

(defn full->normal [tag]
	(->> (::frames tag)
		(group-by :id3.frame/id)
		(m/map-kv-vals tag->normal)))

(defn user-text->full [id [desc values]]
	#:id3.frame{
		:id id
		:description desc
		:content values})

(defn user-url->full [id [desc url]]
	#:id3.frame{
		:id id
		:description desc
		:url url})

(defn picture->full [id content]
	(merge content
		#:id3.frame{
			:id id
			:description ""}))

(defn blob->full [id content]
	#:id3.frame{
		:id id
		:bytes content})

(defn tag->full [[id contents]]
	(case (common/frame-type id)
		:id3.frame.type/text
			[{:id3.frame/id id, :id3.frame/content contents}]
		:id3.frame.type/user-text
			(->> contents
				(mapv #(user-text->full id %))
				(sort-by :id3.frame/description))
		:id3.frame.type/url
			[{:id3.frame/id id, :id3.frame.url contents}]
		:id3.frame.type/user-url
			(->> contents
				(mapv #(user-url->full id %))
				(sort-by :id3.frame/description))
		:id3.frame.type/picture
			(mapv #(picture->full id %) contents)
		:id3.frame.type/blob
			(mapv #(blob->full id %) contents)))
