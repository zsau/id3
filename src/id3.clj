	...
	(into {}
		(for [[id [first-frame & extra-frames :as frames]] (group-by :id3.frame/id (::frames tag))]
			[id (condp = (common/frame-type id)
				...
				:id3.frame.type/user-text (group-frames-by :id3.frame/description frames)
				:id3.frame.type/picture (map #(select-keys % [:id3.frame/picture-type :id3.frame/mime-type :id3.frame/content]) frames)
				(map :id3.frame/content frames))])))
			...
			(apply concat (for [[id contents] tag]
				(condp = (common/frame-type id)
					...
					(for [content contents]
						{:id3.frame/id id :id3.frame/content content}))))))})

(defn ^:private full->simple [tag]
	(let [id->name (set/map-invert (common/frame-names (::version tag)))]
		(into {}
...
(defn ^:private simple->full [version tag]
	(let [name->id (common/frame-names version)]
		(normal->full version
...
"Converts the full-format `tag` to the format specified by `fmt`."
	...
	(let [downconvert (condp = fmt
			:id3.format/full identity
			:id3.format/normal full->normal
			:id3.format/simple full->simple)]
		(downconvert tag)))
...
Options:
  :format  format in which to parse tag (:full, :normal or :simple, default :simple)"
	[istream & {::keys [format]}]
	(downconvert-tag (or format :id3.format/simple)
